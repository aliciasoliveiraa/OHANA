...
def add_susceptibility_artifact(img, susceptibility_factor, blend_factor=0.5):
    ...
    combined = img + (1 - blend_factor) - fat_img * blend_factor
    return np.clip(combined, 0, 255).astype(np.uint8)

def add_aliasing_artifact(img, shift_amount, blend_factor=0.5):
    """Simulates aliasing artifact by undersampling and wrapping the image."""
    fat_img = np.roll(img, shift_amount, axis=0)
    adjusted_blend_factor = 0.5
    combined = np.clip(img + fat_img * blend_factor * adjusted_blend_factor, 0, 255)
...
def apply_random_artifact(img):
    """Applies a random artifact type to the image."""
    artifact_type = np.random.choice(['susceptibility', 'aliasing', 'noise'])

    if artifact_type == 'susceptibility':
        susceptibility_factor = np.random.randint(85, 115)
        blend_factor = np.random.uniform(0.45, 0.55)
        artifact_img = add_susceptibility_artifact(img, susceptibility_factor, blend_factor)
        print("susceptibility")
    elif artifact_type == 'aliasing':
        shift_amount = np.random.randint(125, 155)
        blend_factor = np.random.uniform(0.65, 0.85)
        artifact_img = add_aliasing_artifact(img, shift_amount, blend_factor)
        print("aliasing")
    elif artifact_type == 'noise':
        noise_level = np.random.uniform(5, 15)
        artifact_img = add_noise(img, noise_level)
        print("noise")
...
class MotionSimLayer:
    ...
    def layer_op(self, input_image, translations_rotations=None):
        ...
        if np.random.rand() < 0.3:
            corrupted_artifact = apply_random_artifact(corrupted_im)
